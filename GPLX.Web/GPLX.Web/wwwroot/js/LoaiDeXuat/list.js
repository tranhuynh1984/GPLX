var LoaiDeXuat = function () {
    ...
    base.$txtLoaiDeXuatCode = $('#txtLoaiDeXuatCode');
    base.$txtLoaiDeXuatName = $('#txtLoaiDeXuatName');
    base.$onLoad = true;
    ...
    base.searchForm = function (data) {
        if (typeof (data) !== 'undefined') {
            data.LoaiDeXuatCode = base.$txtLoaiDeXuatCode.val();
            data.LoaiDeXuatName = base.$txtLoaiDeXuatName.val();
            data.Status = $('#ddlStatus option:selected').val();
            base.setParamsAfterSearch(data);

            $('#txtLoaiDeXuatCode').val($.trim(data.LoaiDeXuatCode));
            $('#txtLoaiDeXuatName').val($.trim(data.LoaiDeXuatName));
        }
    }
    ...
    base.exportExcel = function () {
        ...
        costJsBase.ExportExcel('/LoaiDeXuat/ExportExcel', data);
    }
    ...
    base.bindActions = function () {
        ...
        base.$btnCreate.bind("click",
            function () {
                ...
                base.settingPage.fnURI('/LoaiDeXuat/Create');
    ...
    base.tabRecordFuncHandle = function () {
        ...
        if (typeof (propAct) !== 'undefined') {
            switch (propAct) {
                case "pop":
                    switch (action) {
                        ...
                        case "info":
                            ...
                            base.settingPage.fnURI('/LoaiDeXuat/Create?viewMode=view&record=' + jQuery.fn.aesToParams(record.loaiDeXuatCode));
                            base.settingPage.overlay = true;
                            break;
                        case "edit":
                            ...
                            base.settingPage.fnURI('/LoaiDeXuat/Create?viewMode=edit&record=' + jQuery.fn.aesToParams(record.loaiDeXuatCode));
                            base.settingPage.overlay = true;
                            break;
                        default:
                            break;
                ...
                case "confirm":
                    Swal.fire({
                        title: 'Bạn có chắc chắn muốn xóa loại đề xuất này?' + '</br> Bản ghi xóa: ' + record.loaiDeXuatCode,
                        icon: 'warning',
                        ...
                        preConfirm: function () {
                            ...
                            costJsBase.PromissePost({
                                Url: '/LoaiDeXuat/OnRemove',
                                Data: {
                                    loaiDeXuatCode: record.loaiDeXuatCode
                                }
                            }).then(data => {
