var DMBS_ChuyenKhoa = function () {
    ...
    base.$txtMa = $('#txtBSChuyenKhoaCode');
    base.$txtTen = $('#txtBSChuyenKhoaName');
    base.$onLoad = true;
    ...
    base.pageLanguage = languages.vi.DMBS_ChuyenKhoa;
    ...
    base.searchForm = function (data) {
        if (typeof (data) !== 'undefined') {
            data.Ma = base.$txtMa.val();
            data.Ten = base.$txtTen.val();
            data.Status = $('#ddlStatus option:selected').val();
            base.setParamsAfterSearch(data);

            $('#txtMa').val($.trim(data.Ma));
            $('#txtTen').val($.trim(data.Ten));
        }
    }
    ...
    base.exportExcel = function () {
        ...
        costJsBase.ExportExcel('/DMBS_ChuyenKhoa/ExportExcel', data);
    }
        ...
        base.$tableDataTable = $.fn.jsTableRegister({
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function (data, type, row, meta) {
                        let s = '';
                        s = '<a class="mr-2" href="javascript:void(0)" title="Chỉnh sửa thông tin đối tượng" prop-type="elems.table.edit" prop-type-act="pop" ><i class="fad fa-pencil med-med-theme-primary-button"></i></a>'
    ...
    base.bindActions = function () {
        ...
        base.$btnCreate.bind("click",
            function () {
                ...
                base.settingPage.fnURI('/DMBS_ChuyenKhoa/Create');
    ...
    base.tabRecordFuncHandle = function () {
        ...
        if (typeof (propAct) !== 'undefined') {
            switch (propAct) {
                case "pop":
                    switch (action) {
                        ...
                        case "info":
                            ...
                            base.settingPage.fnURI('/DMBS_ChuyenKhoa/Create?viewMode=view&record=' + jQuery.fn.aesToParams(record.ma));
                            base.settingPage.overlay = true;
                            break;
                        case "edit":
                            ...
                            base.settingPage.fnURI('/DMBS_ChuyenKhoa/Create?viewMode=edit&record=' + jQuery.fn.aesToParams(record.ma));
                            base.settingPage.overlay = true;
                            break;
                        default:
                            break;
                ...
                case "confirm":
                    Swal.fire({
                        title: 'Bạn có chắc chắn muốn xóa chuyên khoa này?' + '</br> Bản ghi xóa: ' + record.ma,
                        icon: 'warning',
    ...
    base.onCreate = function (target) {
        costJsBase.Post({
            Url: '/DMBS_ChuyenKhoa/OnCreate', Data: {
                Record: $('#___record').val(),
                IsActive: $('#ddlStatuschuyenkhoa option:selected').val(),
                Ma: $('#txtmachuyenkhoa').val(),
                Ten: $('#txttenchuyenkhoa').val(),
                Stt: $('#txtstt').val()
            },
            beforeSend: function () {
                costJsBase.ButtonState({
