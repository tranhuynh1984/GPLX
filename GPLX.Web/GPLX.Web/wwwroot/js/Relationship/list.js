var Relationship = function () {
    ...
    base.$txtRelationshipCode = $('#txtRelationshipCode');
    base.$txtRelationshipName = $('#txtRelationshipName');
    base.$onLoad = true;
    ...
    base.pageLanguage = languages.vi.Relationship;
    ...
    base.searchForm = function (data) {
        if (typeof (data) !== 'undefined') {
            data.RelationshipCode = base.$txtRelationshipCode.val();
            data.RelationshipName = base.$txtRelationshipName.val();
            data.Status = $('#ddlStatus option:selected').val();
            base.setParamsAfterSearch(data);

            $('#txtRelationshipCode').val($.trim(data.RelationshipCode));
            $('#txtRelationshipName').val($.trim(data.RelationshipName));
        }
    }
        ...
        base.$tableDataTable = $.fn.jsTableRegister({
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function (data, type, row, meta) {
                        let s = '';
                        s = '<a class="mr-2" href="javascript:void(0)" title="Chỉnh sửa thông tin loại đề xuất" prop-type="elems.table.edit" prop-type-act="pop" ><i class="fad fa-pencil med-theme-primary-button"></i></a>'
                        s += '<a class="fs-17 text-info mr-2" prop-type="elems.table.info" prop-type-act="pop" title="Chi tiết loại đề xuất" href="javascript:void(0)"><i class="fad fa-info-square"></i></a>'
                        s += '<a class="text-danger mr-2" prop-type="elems.table.delete" prop-type-act="confirm" title="Xóa loại đề xuất" href="javascript:void(0)"><i class="fad fa-ban"></i></a>'
                        return s;
                    },
                    width: "5%",
                    class: "text-center"
    ...
    base.exportExcel = function () {
        ...
        costJsBase.ExportExcel('/Relationship/ExportExcel', data);
    }
    ...
    base.bindActions = function () {
        ...
        base.$btnCreate.bind("click",
            function () {
                ...
                base.settingPage.fnURI('/Relationship/Create');
    ...
    base.tabRecordFuncHandle = function () {
        ...
        if (typeof (propAct) !== 'undefined') {
            switch (propAct) {
                case "pop":
                    switch (action) {
                        ...
                        case "info":
                            ...
                            base.settingPage.fnURI('/Relationship/Create?viewMode=view&record=' + jQuery.fn.aesToParams(record.relationshipCode));
                            base.settingPage.overlay = true;
                            break;
                        case "edit":
                            ...
                            base.settingPage.fnURI('/Relationship/Create?viewMode=edit&record=' + jQuery.fn.aesToParams(record.relationshipCode));
                            base.settingPage.overlay = true;
                            break;
                        default:
                            break;
                ...
                case "confirm":
                    Swal.fire({
                        title: 'Bạn có chắc chắn muốn xóa quan hệ này?' + '</br> Bản ghi xóa: ' + record.relationshipCode,
                        icon: 'warning',
    ...
    base.onCreate = function (target) {
        costJsBase.Post({
            Url: '/Relationship/OnCreate', Data: {
                Record: $('#___record').val(),
                IsActive: $('#ddlStatusRelationshipCode option:selected').val(),
                RelationshipCode: $('#txtmaRelationship').val(),
                RelationshipName: $('#txttenRelationship').val(),
                Stt: $('#txtstt').val()
            },
            beforeSend: function () {
                costJsBase.ButtonState({
