var DMHuyen = function () {
    ...
    base.$txtMaHuyen = $('#txtMaHuyen');
    base.$txtTenHuyen = $('#txtTenHuyen');
    base.$onLoad = true;
    base.$popOverlay = $('#modal-overlay');
    base.$ddlStatus = $('#ddlStatus option:selected');
    base.$ddlDMTinh = $('#ddlDMTinh option:selected');

    base.recordSelector = null;

    base.recordSelectorIndex = -1;

    base.pageLanguage = languages.vi.DMHuyen;
    ...
    base.searchForm = function (data) {
        if (typeof (data) !== 'undefined') {
            data.MaHuyen = base.$txtMaHuyen.val();
            data.TenHuyen = base.$txtTenHuyen.val();
            data.Status = $('#ddlStatus option:selected').val();
            data.MaTinh = $('#ddlDMTinh option:selected').val();
            base.setParamsAfterSearch(data);

            $('#txtMaHuyen').val($.trim(data.MaHuyen));
            $('#txtTenHuyen').val($.trim(data.TenHuyen));
        }
    }
    ...
    base.exportExcel = function () {
        ...
        costJsBase.ExportExcel('/DMHuyen/ExportExcel', data);
    }
        ...
        if (typeof (propAct) !== 'undefined') {
            switch (propAct) {
                case "pop":
                    ...
                    base.settingPage.fnURI('/DMHuyen/SetInOutView?code=' + record.officesCode);
