var ProfileCK = function () {
    ...
    base.$ProfileMa = $('#txtProfileMa');
    base.$ProfileTen = $('#txtProfileTen');
    base.$onLoad = true;
    ...
    base.searchForm = function (data) {
        if (typeof (data) !== 'undefined') {
            data.ProfileCKMa = base.$ProfileMa.val();
            data.ProfileCKTen = base.$ProfileTen.val();
            data.Status = $('#ddlStatus option:selected').val();
            data.ChuyenKhoaMa = $('#ddlDMChuyenKhoa option:selected').val();
            base.setParamsAfterSearch(data);

            $('#txtProfileCKMa').val($.trim(data.ProfileCKMa));
            $('#txtProfileCKTen').val($.trim(data.ProfileCKTen));
        }
    }
        ...
        base.$tableDataTable = $.fn.jsTableRegister({
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function (data, type, row, meta) {
                        let s = '';
                        s = '<a class="mr-2" href="javascript:void(0)" title="Chỉnh sửa thông tin profile ck" prop-type="elems.table.edit" prop-type-act="pop" ><i class="fad fa-pencil med-theme-primary-button"></i></a>'
                        s += '<a class="text-danger mr-2" prop-type="elems.table.delete" prop-type-act="confirm" title="Xóa profile ck" href="javascript:void(0)"><i class="fad fa-ban"></i></a>'
                        return s;
                    },
                    width: "5%",
                    class: "text-center"
    ...
    base.bindActions = function () {
        ...
        base.$btnCreate.bind("click",
            function () {
                window.location.href = '/ProfileCK/Create';
            });
            ...
            switch (propAct) {
                case "pop":
                    switch (action) {
                        ...
                        case "edit":
                            window.location.href = '/ProfileCK/Create?ProfileCKMa=' + record.profileCKMa;
                            break;
                        default:
                            break;
                    }
                ...
                case "confirm":
                    Swal.fire({
                        title: 'Bạn có chắc chắn muốn xóa profile ck này?' + '</br> Bản ghi xóa: ' + record.profileCKMa,
                        icon: 'warning',
