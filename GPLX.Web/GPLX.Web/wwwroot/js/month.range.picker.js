...
$(function () {
    ...
    if (calendarCount == 0)
        calendarCount++;
    for (y = 0; y < 2; y++) {
        let rader = costJsBase.Rad(10);
        content += '<div class="col-md-6 col-sm-5"><div class="mpr-calendar row" id="mpr-calendar-' + (y + 1) + '">'
            + '<h5 class="col-sm-12"><i class="mpr-yeardown fa fa-chevron-circle-left"></i><span>' + (startYear + y).toString() + '</span><i class="mpr-yearup fa fa-chevron-circle-right"></i></h5><div class="mpr-monthsContainer"><div class="mpr-MonthsWrapper">';
        for (m = 0; m < 12; m++) {
            var monthval;
            if ((m + 1) < 10)
                monthval = "0" + (m + 1);
                ...
                monthval = "" + (m + 1);
            content += '<span id="' + rader + ':' + monthval + '" value="' + monthval + '" data-month="' + monthval + '" class="col-sm-3 text-center mpr-month">' + MONTHS[m] + '</span>';
        }
        content += '</div></div></div></div>';
    }
    content += '<div class="col-sm-2 hidden-sm-down"> <h5  class="mpr-quickset">Chọn nhanh</h5>';
    content += '<a class="btn btn-info mpr-fiscal-ytd">Fiscal YTD</a>';
    content += '<a class="btn btn-info mpr-ytd">YTD</a>';
    content += '<a class="btn btn-info mpr-prev-fiscal">Previous FY</a>';
    content += '<a class="btn btn-info mpr-prev-year">Năm trước</a>';
    content += '</div>';
    content += '</div>';
    ...
    $(document).on('click', '.mpr-month', function (e) {
        e.stopPropagation();
        $month = $(this);
        let fieldId = $month.attr('id');
        var monthnum = fieldId.split(':')[1];
