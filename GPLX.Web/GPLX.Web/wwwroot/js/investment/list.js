var InvestmentList = function () {
    ...
    base.pageLanguage = languages.vi.investment;
        ...
        base.$tableDataTable = $.fn.jsTableRegister({
            ...
            columns: [
                ...
                },
                {
                    idx: 5,
                    render: function (data, type, row, meta) {
                        return costJsBase.FormatMoney(row.totalExpectCostInvestment, '-');
                    },
                    class: "text-right",
                    width: "10%"
                },
                {
                    idx: 6,
                    render: function (data, type, row, meta) {
                        return costJsBase.FormatMoney(row.totalExpenditureCapital, '-');
                    },
                    class: "text-right",
                    width: "10%"
                },
                {
                    idx: 7,
                    render: function (data, type, row, meta) {
                        return costJsBase.FormatMoney(row.totalCapitalMedGroup, '-');
                    },
                    class: "text-right",
                    width: "10%"
                },
                {
                    idx: 8,
                    render: function (data, type, row, meta) {
                        return costJsBase.FormatMoney(row.totalSpendingLoan, '-');
                    },
                    class: "text-right",
                    width: "10%"
                },
                ...
                },
                {
                    ...
                    render: function (data, type, row, meta) {
                        ...
                        if (row.approvalable)
                            s +=
                                '<a class="fs-17 mr-2 text-info" href="javascript:void(0)" title="Phê duyệt kế hoạch đầu tư" prop-type="elems.table.approve" prop-type-act="confirm" >' +
                                '<i class="fad fa-check"></i></a>';
                        if (row.declineable)
                            s +=
                                '<a class="fs-17 mr-2 text-danger" href="javascript:void(0)" title="Từ chối kế hoạch đầu tư" prop-type="elems.table.decline" prop-type-act="confirm">' +
                                '<i class="far fa-ban"></i></a>';
                        if (row.deleteable)
                            s +=
                                '<a class="fs-17 mr-2 text-danger" href="javascript:void(0)" title="Xóa kế hoạch đầu tư" prop-type="elems.table.delete" prop-type-act="confirm">' +
                ...
                case "pop":
                    switch (action) {
                        ...
                        case "history":
                            ...
                            base.settingPage.fnURI('/InvestmentPlan/ViewHistories?record=' + jQuery.fn.aesToParams(record.record));
    ...
    base.onAprove = function (point) {
        Swal.fire({
            title: 'Bạn có chắc chắn muốn phê duyệt kế hoạch này ?',
            icon: 'success',
    ...
    base.onDecline = function (point) {
        ...
        Swal.fire({
            title: 'Bạn có chắc chắn muốn từ chối kế hoạch này ?',
            icon: 'error',
            showDenyButton: true,
